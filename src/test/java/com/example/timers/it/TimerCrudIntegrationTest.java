...
public class TimerCrudIntegrationTest extends BaseIntegrationTest {
    ...
    void createActiveTimer_persistsTimersAndQuartz_withWaitingState() throws Exception {
        ...
        MvcResult res = mockMvc.perform(post("/api/templates")
    ...
    void bulkSuspendResume_twoTimers() throws Exception {
        ...
        String id1 = mockMvc.perform(post("/api/templates").contentType(MediaType.APPLICATION_JSON).content(body1))
                .andExpect(status().isCreated()).andReturn().getResponse().getContentAsString()
                .replaceAll(".*\"id\":\"([^\"]+)\".*", "$1");
        String id2 = mockMvc.perform(post("/api/templates").contentType(MediaType.APPLICATION_JSON).content(body2))
                .andExpect(status().isCreated()).andReturn().getResponse().getContentAsString()
                .replaceAll(".*\"id\":\"([^\"]+)\".*", "$1");
        ...
        mockMvc.perform(put("/api/templates/" + id1).contentType(MediaType.APPLICATION_JSON).content(suspend1)).andExpect(status().isOk());
        mockMvc.perform(put("/api/templates/" + id2).contentType(MediaType.APPLICATION_JSON).content(suspend2)).andExpect(status().isOk());
        ...
        mockMvc.perform(put("/api/templates/" + id1).contentType(MediaType.APPLICATION_JSON).content(resume1)).andExpect(status().isOk());
        mockMvc.perform(put("/api/templates/" + id2).contentType(MediaType.APPLICATION_JSON).content(resume2)).andExpect(status().isOk());
    ...
    void manualTrigger_createsManualExecution() throws Exception {
        ...
        String id = mockMvc.perform(post("/api/templates").contentType(MediaType.APPLICATION_JSON).content(body))
                .andExpect(status().isCreated()).andReturn().getResponse().getContentAsString()
                .replaceAll(".*\"id\":\"([^\"]+)\".*", "$1");
    ...
    void severalManualTriggers_createExecutionsForEach() throws Exception {
        String id1 = mockMvc.perform(post("/api/templates").contentType(MediaType.APPLICATION_JSON)
                        .content(TimerRequests.create("mm1", "UTC", "10:15", false)))
                .andExpect(status().isCreated()).andReturn().getResponse().getContentAsString()
                .replaceAll(".*\"id\":\"([^\"]+)\".*", "$1");
        String id2 = mockMvc.perform(post("/api/templates").contentType(MediaType.APPLICATION_JSON)
